 ...
 */
@Service
public class BaseSwDemoServiceImpl extends ServiceImpl<JeecgDemoMapper, BaseSwDemo> implements IBaseSwDemoService {
    @Autowired
    ...
    public void testTran() {
        BaseSwDemo pp = new BaseSwDemo();
        pp.setAge(1111);
        pp.setName("测试事务  小白兔 1");
        jeecgDemoMapper.insert(pp);

        BaseSwDemo pp2 = new BaseSwDemo();
        pp2.setAge(2222);
    ...
    public BaseSwDemo getByIdCacheable(String id) {
        BaseSwDemo t = jeecgDemoMapper.selectById(id);
    ...
    @Override
    public IPage<BaseSwDemo> queryListWithPermission(int pageSize, int pageNo) {
        Page<BaseSwDemo> page = new Page<>(pageNo, pageSize);
        //编程方式，获取当前请求的数据权限规则SQL片段
        String sql = QueryGenerator.installAuthJdbc(BaseSwDemo.class);
