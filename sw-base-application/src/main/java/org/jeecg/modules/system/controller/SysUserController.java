...
public class SysUserController {
    ...
    @ApiOperation(value = "用户列表-分页列表查询(含权限)", notes = "用户列表-分页列表查询(含权限)")
    public Result<?> queryPageList(SysUser user, @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
        ...
        List<String> userIds = sysUserVoList.stream().map(SysUserVo::getId).collect(Collectors.toList());
        if (userIds != null && userIds.size() > 0) {
            Map<String, SysUserDepVo> useDepNames = sysUserService.getDepFullNamesByUserIds(userIds);
            Map<String, SysUserRolesVo> roleNames = sysUserService.getRoleNamesByUserIds(userIds);
            sysUserVoList.forEach(item -> {
                SysUserDepVo sysUserDepVo = useDepNames.get(item.getId());
                SysUserRolesVo sysUserRolesVo = roleNames.get(item.getId());
                item.setOrgCodeTxt(sysUserDepVo != null ? sysUserDepVo.getDepartName() : "");
                item.setOrgCodeFullTxt(sysUserDepVo != null ? sysUserDepVo.getDepartFullName() : "");
                item.setRoleCode(sysUserRolesVo != null ? sysUserRolesVo.getRoleCode() : "");
                item.setRoleName(sysUserRolesVo != null ? sysUserRolesVo.getRoleName() : "");
            });
        }
        Page<SysUserVo> sysUserVoPage = new Page<SysUserVo>(pageNo, pageSize);
        sysUserVoPage.setRecords(sysUserVoList).setTotal(pageList.getTotal());
        return Result.ok(sysUserVoPage);
    }

    @AutoLog(value = "用户列表-分页列表查询(含权限)")
    @ApiOperation(value = "用户列表-分页列表查询(含权限)", notes = "用户列表-分页列表查询(含权限)")
    @RequestMapping(value = "/querySysDeptUserList", method = RequestMethod.GET)
    public Result<?> querySysDeptUserList(SysUserVo user,
        @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
        @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize, HttpServletRequest req) {
        Map<String, String[]> map = req.getParameterMap();
        Map<String, String[]> paramsMap = new HashMap<>();
        //移除createTime 防止重复表值sql注入报错
        for (Map.Entry<String, String[]> entry : map.entrySet()) {
            String key = entry.getKey();
            String[] value = entry.getValue();
            if (!(StringUtils.equalsIgnoreCase("createTime_begin", key) || StringUtils
                .equalsIgnoreCase("createTime_end", key))) {
                paramsMap.put(key, value);
            }
        }
        String code = user.getDepartOrgCode();
        Integer status = user.getStatus();
        user.setDepartOrgCode(null);
        user.setStatus(null);
        QueryWrapper<SysUserVo> queryWrapper = QueryGenerator.initQueryWrapper(user, paramsMap);
        if (oConvertUtils.isNotEmpty(code)) {
            queryWrapper.like("sd.org_code", code.replace("*", "%"));
        }
        if (oConvertUtils.isNotEmpty(status)) {
            queryWrapper.eq("suser.status", status);
        }
        //手动添加时间过滤条件
        if (map.get("createTime_begin") != null) {
            String beginValue = map.get("createTime_begin")[0].trim();
            queryWrapper.gt("suser.create_time", beginValue);
        }
        if (map.get("createTime_end") != null) {
            String endValue = map.get("createTime_end")[0].trim();
            queryWrapper.le("suser.create_time", endValue);
        }
        //TODO 外部模拟登陆临时账号，列表不显示
        queryWrapper.ne("username", "_reserve_user_external");
        //排序逻辑 处理
        ...
        if (oConvertUtils.isEmpty(column) && oConvertUtils.isEmpty(order)) {
            queryWrapper.orderByDesc("suser.create_time");
        }
        Page<SysUserVo> page = new Page<SysUserVo>(pageNo, pageSize);
        List<SysUserVo> pageList = sysUserService.querySysDeptUserList(page, queryWrapper);
        //批量查询用户的所属部门0
        //step.1 先拿到全部的 useids
        //step.2 通过 useids，一次性查询用户的所属部门名字
        List<String> userIds = pageList.stream().map(SysUserVo::getId).collect(Collectors.toList());
        if (userIds != null && userIds.size() > 0) {
            Map<String, SysUserDepVo> useDepNames = sysUserService.getDepFullNamesByUserIds(userIds);
            Map<String, SysUserRolesVo> userRolesNames = sysUserService.getRoleNamesByUserIds(userIds);
            pageList.forEach(item -> {
                SysUserDepVo sysUserDepVo = useDepNames.get(item.getId());
                SysUserRolesVo sysUserRolesVo = userRolesNames.get(item.getId());
                //判断非空
                item.setOrgCodeTxt(sysUserDepVo != null ? sysUserDepVo.getDepartName() : "");
                item.setOrgCodeFullTxt(sysUserDepVo != null ? sysUserDepVo.getDepartFullName() : "");
                item.setRoleCode(sysUserRolesVo != null ? sysUserRolesVo.getRoleCode() : "");
    ...
    @AutoLog(value = "修改头像-编辑")
    @ApiOperation(value = "修改头像-编辑", notes = "修改头像-编辑")
    ...
    public Result<?> updateAvatar(@RequestBody SysUser sysUser) {
        ...
        sysUserService.updateUserAvatar(sysUser);
        return Result.ok("修改成功");
    }

    @RequestMapping(value = "/edit", method = RequestMethod.PUT)
    public Result<SysUser> edit(@RequestBody JSONObject jsonObject) throws Exception {
    ...
    public Result<?> userRoleList(@RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
        ...
        Page<SysUserVo> sysUserVoPage = getSysUserVoPage(pageNo, pageSize, pageList);
        return Result.ok(sysUserVoPage);

    }

    private Page<SysUserVo> getSysUserVoPage(@RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
        @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize, IPage<SysUser> pageList) {
        List<SysUserVo> sysUserVoList = new ArrayList<SysUserVo>();
        for (SysUser sysUser : pageList.getRecords()) {
            SysUserVo sysUserVo = new SysUserVo();
            BeanUtils.copyProperties(sysUser, sysUserVo);
     ...
     * 查询某个角色下面所有的用户
     */
    @AutoLog(value = "用户列表-查询用户角色")
    @ApiOperation(value = "用户列表-查询用户角色", notes = "用户列表-查询用户角色")
    @RequestMapping(value = "/queryUserInfoByRoleCode", method = RequestMethod.GET)
    public Result<?> queryUserInfoByRoleCode(String roleCodes, String orgCode, String realname,
        @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
        @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize, HttpServletRequest req) {
        Page<SysUserInfoRoleVo> page = new Page<SysUserInfoRoleVo>(pageNo, pageSize);
        List<SysUserInfoRoleVo> pages = sysUserService.queryUserInfoByRoleCode(page, roleCodes, orgCode, realname);
        page.setRecords(pages);
        return Result.ok(page);
    }

    @AutoLog(value = "用户列表-查询坐席组别和平台")
    @ApiOperation(value = "用户列表-查询坐席组别和平台", notes = "用户列表-查询坐席组别和平台")
    @RequestMapping(value = "/queryUserDepartAndBp", method = RequestMethod.GET)
    public Result<?> queryUserDepartAndBp(String roleCodes, String orgCode, String realname,
        @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
        @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize, HttpServletRequest req) {
        Page<SysUserInfoRoleVo> page = new Page<SysUserInfoRoleVo>(pageNo, pageSize);
        List<SysUserInfoRoleVo> pages = sysUserService.queryUserInfoByRoleCode(page, roleCodes, orgCode, realname);
        ...
        List<SysDepartModel> sysDepartBpModelList =
            sysBaseAPI.getDepartsByCode(new SysDepartModel().setDepType(2 + ""));

        pages.forEach(item -> {
            String itemOrgCode = item.getOrgCode();
            for (SysDepartModel sysDepartModel : sysDepartBpModelList) {
                String sysDepartOrgCode = sysDepartModel.getOrgCode();
                if (itemOrgCode.indexOf(sysDepartOrgCode) != -1) {
                    item.setBpOrgCode(sysDepartOrgCode);
                    item.setBpOrgName(sysDepartModel.getDepartName());
                    break;
                }
            }
        });
     ...
     */
    @AutoLog(value = "用户列表-查询用户角色")
    @ApiOperation(value = "用户列表-查询部门下所有的用户", notes = "用户列表-查询用户角色")
    @RequestMapping(value = "/querySysGroupUserInfo", method = RequestMethod.GET)
    public Result<?> querySysGroupUserInfo(String roleCodes, String orgCode, String realname, HttpServletRequest req) {
        List<SysDepartGroupUserVo> pages = sysUserService.queryByGroupUserList(roleCodes, orgCode, realname);
        return Result.ok(pages);
    }

    @ApiOperation(value = "修改密码", notes = "修改密码")
    @RequestMapping(value = "/batchUpdatePassWord", method = RequestMethod.POST)
    public Result<?> batchUpdatePassWord(String password) {
        sysUserService.batchUpdatePassWord(password);
        return Result.ok("修改成功");
    }
