...
import org.jeecg.common.system.vo.SysUserDepartVO;
...
public class SysDepartServiceImpl extends ServiceImpl<SysDepartMapper, SysDepart> implements ISysDepartService {
    ...
    @Override
    public List<SysDepart> getSupDepart(SysDepart sysDepart) {
        List<String> list = null;
        if (sysDepart.getOrgCode() != null) {
            String departCode = sysDepart.getOrgCode();
            list = this.baseMapper.getSupDepIdsByDepCode(departCode);
            sysDepart.setOrgCode(null);
        } else if (sysDepart.getId() != null) {
            String departId = sysDepart.getId();
            list = this.baseMapper.getSupDepIdsByDepId(departId);
        }
        // 查询下级部门过滤字段类型 ，如depType =2,orgType=1
        if (list != null && list.size() > 0) {
            String ids = String.join(",", list);
            sysDepart.setId("," + ids + ",");
        }
        return getDepartsByCode(sysDepart);
    }

    @Override
    public List<SysDepart> getDepartsByCode(SysDepart sysDepart) {
        //清除权限
        JeecgDataAutorUtils.clearSearchCondition();
     ...
     */
    @Override
    public List<SysDepartTreeModel> queryTreeByOrgCode(String orgCode) {
        LambdaQueryWrapper<SysDepart> query = new LambdaQueryWrapper<SysDepart>();
        query.eq(SysDepart::getDelFlag, CommonConstant.DEL_FLAG_0.toString()).
            likeRight(SysDepart::getOrgCode, orgCode)
            .and(i -> i.eq(SysDepart::getDepType, 3).or().like(SysDepart::getDepType, 2));
        query.orderByAsc(SysDepart::getDepartOrder);
        List<SysDepart> list = this.list(query);
        List<SysDepartTreeModel> listResult = new LinkedList<>();
        List<SysDepart> listPub =
            list.stream().filter(resp -> StringUtils.equals(resp.getDepType(), 2 + "")).collect(Collectors.toList());

        //添加树形结构
        for (SysDepart sysDepart : listPub) {
            SysDepartTreeModel sysDepartTreeModel = new SysDepartTreeModel();
            BeanUtils.copyProperties(sysDepart, sysDepartTreeModel);
            ...
            List<SysDepart> listPubChild = list.stream().filter(
                resp -> (StringUtils.equals(resp.getDepType(), 3 + "") && resp.getParentId().equals(sysDepart.getId())))
                .collect(Collectors.toList());
            List<SysDepartTreeModel> child = sysDepartTreeModel.getChildren();
            for (SysDepart sysDepartChild : listPubChild) {
                SysDepartTreeModel sysDepartChildModel = new SysDepartTreeModel();
                BeanUtils.copyProperties(sysDepartChild, sysDepartChildModel);
                sysDepartChildModel.setKey(sysDepartChild.getId());
                sysDepartChildModel.setValue(sysDepartChild.getId());
                sysDepartChildModel.setTitle(sysDepartChild.getDepartName());
                sysDepartChildModel.setChildren(null);//下级处理完
                child.add(sysDepartChildModel);
            }
     ...
     * 修改部门
     *
     * @param sysUserDepartVO
     */
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void updateDepart(SysUserDepartVO sysUserDepartVO) {
        SysUserDepart sysUserDepart = new SysUserDepart();
        BeanUtils.copyProperties(sysUserDepartVO, sysUserDepart);

        LambdaQueryWrapper<SysUserDepart> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        lambdaQueryWrapper.eq(SysUserDepart::getUserId, sysUserDepart.getUserId())
            .eq(SysUserDepart::getDepId, sysUserDepartVO.getSourceDepId());

        userDepartMapper.update(sysUserDepart, lambdaQueryWrapper);
    }
    ...
    public String queryDepartCode(String code, String type) {
        ...
        lambdaQueryWrapper.eq(SysDepart::getOrgCode, code).eq(SysDepart::getDepType, type);
        SysDepart sysDepart = this.getOne(lambdaQueryWrapper);
        String depId = sysDepart.getId();
        return depId;
    }

    @Override
    public List<SysUserDepartInfoVo> getUserBySysDeparts(List<String> workNos) {
        return this.baseMapper.getUserBySysDeparts(workNos);
    }
}
